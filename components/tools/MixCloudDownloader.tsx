...
const Loader: React.FC = () => (
    <div className="text-center">
        <div className="mixcloud-loader mx-auto">
            <div className="cloud"></div>
            <div className="wave"></div>
        </div>
        <p className="text-lg font-semibold text-slate-600 dark:text-slate-300 mt-6">Tuning in to Mixcloud...</p>
        <style>{`
            .mixcloud-loader {
                width: 100px;
                height: 80px;
            ...
            .wave {
                ...
                width: 80px;
                height: 20px;
                background-image: linear-gradient(90deg, transparent 50%, white 50%), linear-gradient(90deg, transparent 50%, white 50%);
                background-size: 8px 100%, 8px 100%;
                background-position: 0 0, 4px 0;
                background-repeat: repeat-x;
                mask-image: linear-gradient(to top, black, transparent);
                animation: wave-move 1s infinite linear;
            }
            @keyframes wave-move {
                from { background-position: 0 0, 4px 0; }
                to { background-position: -8px 0, -4px 0; }
            }
        `}</style>
    </div>
);

export const MixCloudDownloader: React.FC<{ title: string }> = ({ title }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">Enter the URL of the Mixcloud audio you want to download.</p>
      </div>
      ...
      <div className="p-8 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
        {isLoading ? (
            ...
            <form onSubmit={handleAttempt} className="space-y-4">
                <div>
                    <label htmlFor="mc-url" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                        Mixcloud URL
                    </label>
                    <input
                        id="mc-url"
