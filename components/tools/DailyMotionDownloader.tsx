...
const Loader: React.FC = () => (
    <div className="text-center">
        <div className="dm-loader mx-auto">
            <div className="dm-shape"></div>
        </div>
        <p className="text-lg font-semibold text-slate-600 dark:text-slate-300 mt-6">Connecting to DailyMotion...</p>
        <style>{`
            .dm-loader {
                width: 80px;
                height: 80px;
            ...
            .dm-shape {
                width: 100%;
                height: 100%;
                background-color: #0062FF; /* DailyMotion Blue */
                clip-path: polygon(0% 0%, 100% 0%, 100% 75%, 50% 100%, 0% 75%);
                animation: pulse-dm 1.5s infinite ease-in-out;
            }
            @keyframes pulse-dm {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">Enter the URL to get started.</p>
      </div>
      ...
      <div className="p-8 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
        {isLoading ? (
            ...
            <form onSubmit={handleAttempt} className="space-y-4">
                <div>
                    <label htmlFor="dm-url" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
