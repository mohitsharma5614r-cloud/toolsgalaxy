...
export const GifSplitter: React.FC<{ title: string }> = ({ title }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-8 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
      ...
      <p className="text-slate-600 dark:text-slate-300 text-lg">This tool requires a specialized third-party library.</p>
      <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-2xl">
        Parsing and splitting animated GIF files frame-by-frame is a complex task that requires a dedicated JavaScript library (like `gifuct-js` or `gifler`). 
        Since this library is not included in the current environment's import map, this tool cannot be implemented.
      </p>
    </div>
  );
};
