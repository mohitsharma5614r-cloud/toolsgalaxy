...
const Loader: React.FC = () => (
    <div className="text-center">
        ...
        <p className="text-lg font-semibold text-slate-600 dark:text-slate-300 mt-8">Generating your invoice...</p>
        <style>{`
            ...
            .receipt-paper { width: 40px; height: 50px; background: #f1f5f9; position: absolute; top: -30px; left: 50%; transform: translateX(-50%); animation: print-receipt 2s infinite linear; }
            .drawer { width: 110%; height: 30px; background: #64748b; border-radius: 0 0 8px 8px; position: absolute; bottom: -30px; left: -5%; display: flex; justify-around; align-items: center; animation: open-drawer 2s infinite; }
            .dark .drawer { background: #94a3b8; }
            .coin { width: 15px; height: 15px; background: #facc15; border-radius: 50%; }
            @keyframes print-receipt { 0% { height: 10px; top: 5px; } 80%, 100% { height: 50px; top: -30px; } }
            @keyframes open-drawer { 0%, 20%, 80%, 100% { transform: translateY(0); } 40%, 60% { transform: translateY(10px); } }
        `}</style>
    </div>
);
...
export const InvoicePdfAutoFiller: React.FC<{ title: string }> = ({ title }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="text-center mb-10">
                ...
                <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg">Automatically fill out invoice templates with your data.</p>
            </div>
                        ...
                        <div className="space-y-2 pt-4">
                            ...
                            {items.map((item) => (
                                <div key={item.id} className="grid grid-cols-[1fr,80px,100px,30px] gap-2 items-center">
                                    <input value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)} placeholder="Item or service" className="input-style" />
                                    <input type="number" value={item.quantity} onChange={e => updateItem(item.id, 'quantity', Number(e.target.value))} placeholder="1" className="input-style text-center"/>
                                    <input type="number" value={item.rate} onChange={e => updateItem(item.id, 'rate', Number(e.target.value))} placeholder="0.00" className="input-style text-right"/>
            ...
            {error && <Toast message={error} onClose={() => setError(null)} />}
             <style>{`
                ...
                .btn-secondary { background-color: #64748b; color: white; border-radius: 0.5rem; font-weight: 600; padding: 0.5rem 1rem; transition: background-color 0.2s; }
            `}</style>
        </div>
    );
};
